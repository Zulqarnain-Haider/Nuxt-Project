<template>
  <div class="min-h-screen text-mainText py-10 px-6 md:px-16 bg-bgDark">
    <div class="max-w-7xl mx-auto flex flex-col lg:flex-row gap-10 font-poppins">
      ...
      <div class="flex-1 space-y-6">
        <div
          ...
          :key="item.id"
          class="flex flex-col md:flex-row gap-5 border border-onFooter/80 rounded-xl bg-bgLight/20 p-5 relative"
        >
          <!--Remove Button -->
          <button
            @click="removeItem(item.id)"
            class="absolute top-3 right-3 text-onMainText/70 hover:text-red-500 transition"
            title="Remove item"
          >
  ...
  <span class="text-lg text-yellow-400">{{ item.rating }}<span class="text-onFooter text-sm">/100</span></span>
  <span class="text-green-600 font-medium">{{ item.stockStatus }}</span>
        ...
        <p
          ...
          class="text-center text-onMainText/60 mt-16 text-lg"
        >
      ...
      <!-- Right: Order Summary -->
      <div
        class="w-full lg:w-[350px] bg-bgDark/70 border border-outline rounded-xl p-6 h-fit self-start"
      >
        ...
        <AppButton
          variant="primary"
          extraClass="w-full mt-6 py-3 px-12 
